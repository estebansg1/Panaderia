...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["action"]) && $_POST["action"] == "eliminar") {
    if (isset($_POST["rut_proveedor"])) {
        ...
        $sql = "DELETE FROM proveedor WHERE rut_proveedor='$rut_proveedor'";
...
<body>
    <header>
        ...
        if ($result->num_rows > 0) {
            echo "<table border='1'>";
            echo "<tr><th>Rut Proveedor</th><th>Nombre</th><th>Teléfono 1</th><th>Teléfono 2</th><th>Número de Calle</th><th>Calle</th><th>Ciudad</th><th>Colonia</th><th>Acciones</th></tr>";
            
            while($row = $result->fetch_assoc()) {
                echo "<tr>";
                echo "<td>" . $row["rut_proveedor"] . "</td>";
                echo "<td>" . $row["nombre_p"] . "</td>";
                echo "<td>" . $row["telefono1_p"] . "</td>";
                echo "<td>" . $row["telefono2_p"] . "</td>";
                echo "<td>" . $row["numero_calle_p"] . "</td>";
                echo "<td>" . $row["calle_p"] . "</td>";
                echo "<td>" . $row["ciudad_p"] . "</td>";
                echo "<td>" . $row["colonia_p"] . "</td>";
                
                // Agregar botones de edición y eliminación
                echo "<td>
                        <form action='editarproveedor.php' method='GET' style='display:inline;'>
                            <input type='hidden' name='rut_proveedor' value='" . $row["rut_proveedor"] . "'>
            ...
            echo "No hay registros en la tabla de proveedores.";
        }
        ?>
        <button class="buttonadd">
            <a href="addproveedor.php">Agregar Nuevo Proveedor</a>
        </button>
    </div>
</body>
</html>